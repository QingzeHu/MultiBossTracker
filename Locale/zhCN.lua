-- MultiBossTracker zhCN locale
local addonName, MBT = ...

MBT:RegisterLocale("zhCN", {
    -- Testing
    ["The Great Forge"] = "大锻炉",
    ["Valley of Honor"] = "荣誉谷",
    ["Heroic Training Dummy"] = "英雄训练假人",
    ["Grandmaster's Training Dummy"] = "宗师的训练假人",
    ["Expert's Training Dummy"] = "专家的训练假人",
    ["Master's Training Dummy"] = "大师的训练假人",

    -- Ulduar
    ["Formation Grounds"] = "练兵场",
    ["Flame Leviathan"] = "烈焰巨兽",
    ["The Colossal Forge"] = "巨人熔炉",
    ["Ignis the Furnace Master"] = "掌炉者伊格尼斯",
    ["Razorscale's Aerie"] = "锋鳞之巢",
    ["Razorscale"] = "锋鳞",
    ["The Scrapyard"] = "废料场",
    ["XT-002 Deconstructor"] = "XT-002拆解者",
    ["The Celestial Planetarium"] = "天文台",
    ["Algalon the Observer"] = "观察者奥尔加隆",
    ["The Observation Ring"] = "观测场",
    ["Auriaya"] = "欧尔莉亚",
    ["The Halls of Winter"] = "寒冬大厅",
    ["Hodir"] = "霍迪尔",
    ["The Clash of Thunder"] = "雷霆角斗场",
    ["Thorim"] = "托里姆",
    ["The Descent into Madness"] = "疯狂阶梯",
    ["General Vezax"] = "维扎克斯将军",
    ["The Prison of Yogg-Saron"] = "尤格-萨隆的监狱",
    ["Yogg-Saron"] = "尤格-萨隆",
    ["The Assembly of Iron"] = "钢铁议会",
    ["Stormcaller Brundir"] = "唤雷者布隆迪尔",
    ["Runemaster Molgeim"] = "符文大师莫尔基姆",
    ["Steelbreaker"] = "断钢者",
    ["The Shattered Walkway"] = "破碎通道",
    ["Right Arm"] = "右臂",
    ["Kologarn"] = "科隆加恩",
    ["Left Arm"] = "左臂",
    ["The Conservatory of Life"] = "生命温室",
    ["Snaplasher"] = "迅疾鞭笞者",
    ["Storm Lasher"] = "风暴鞭笞者",
    ["Ancient Water Spirit"] = "古代水之精魂",
    ["The Spark of Imagination"] = "思想火花",
    ["Aerial Command Unit"] = "空中指挥单位",
    ["Leviathan Mk II"] = "巨兽二型",
    ["Saronite Animus"] = "萨隆邪铁畸体",
    ["Life Spark"] = "生命火花",
    ["XM-024 Pummeller"] = "XM-024击打者",
    ["Freya"] = "弗蕾亚",
    ["Assault Bot"] = "突击机器人",
    ["Crusher Tentacle"] = "重压触须",
    ["Corruptor Tentacle"] = "腐蚀触须",
    ["Feral Defender"] = "野性防御者",

    -- TOC + Onyxia
    ["Trial of the Crusader"] = "十字军的试炼",
    ["Gormok the Impaler"] = "穿刺者戈莫克",
    ["Snobold Vassal"] = "雪地狗头人奴隶",
    ["Acidmaw"] = "酸喉",
    ["Dreadscale"] = "恐鳞",
    ["Icehowl"] = "冰吼",
    ["Lord Jaraxxus"] = "加拉克苏斯大王",
    ["Mistress of Pain"] = "痛苦女王",
    ["Felflame Infernal"] = "魔焰地狱火",
    ["Shaabad"] = "沙拜德",
    ["Broln Stouthorn"] = "布罗恩·粗角",
    ["Anthar Forgemender"] = "安塔尔·缮炉者",
    ["Caiphus the Stern"] = "严肃的凯普斯",
    ["Melador Valestrider"] = "麦拉多·深谷游者",
    ["Erin Misthoof"] = "伊林·雾蹄",
    ["Saamul"] = "萨缪尔",
    ["Thrakgar"] = "萨卡加尔",
    ["Irieth Shadowstep"] = "伊锐丝·影踪",
    ["Maz'dinah"] = "玛兹迪娜",
    ["Shocuul"] = "索库尔",
    ["Narrhok Steelbreaker"] = "断钢者纳霍克",
    ["Alyssia Moonstalker"] = "阿莱希娅·月行者",
    ["Ruj'kah"] = "鲁姬卡",
    ["Tyrius Duskblade"] = "泰利乌斯·达斯布雷德",
    ["Gorgrim Shadowcleave"] = "戈瑞姆·影斩",
    ["Serissa Grimdabbler"] = "塞瑞莎·术轮",
    ["Harkzog"] = "德拉克道格",
    ["Baelnor Lightbearer"] = "圣光使者巴尔诺",
    ["Malithas Brightblade"] = "玛里萨斯·辉刃",
    ["Velanaa"] = "维兰纳",
    ["Liandra Suncaller"] = "莉安德拉·唤日者",
    ["Kavina Grovesong"] = "卡雯娜·林歌",
    ["Birana Stormhoof"] = "比莱纳·雷蹄",
    ["Brienna Nightfell"] = "布瑞娜·沉夜",
    ["Vivienne Blackwhisper"] = "暗语者维维尼",
    ["Noozle Whizzlestick"] = "努兹尔·啸钉",
    ["Ginselle Blightslinger"] = "凋零者吉塞尔",
    ["Fjola Lightbane"] = "光明邪使菲奥拉",
    ["Eydis Darkbane"] = "黑暗邪使艾蒂丝",
    ["The Icy Depths"] = "寒冰深渊",
    ["Anub'arak"] = "阿努巴拉克",
    ["Nerubian Burrower"] = "蛛魔掘地者",
    ["Onyxia's Lair"] = "奥妮克希亚的巢穴",
    ["Onyxia"] = "奥妮克希亚",
    ["Onyxian Lair Guard"] = "奥妮克希亚巢穴守卫",

    -- ICC
    ["The Spire"] = "冰冠之塔",
    ["Lord Marrowgar"] = "玛洛加尔领主",
    ["Bone Spike"] = "骨针",
    ["Oratory of the Damned"] = "诅咒祷堂",
    ["Lady Deathwhisper"] = "亡语者女士",
    ["Cult Adherent"] = "教派追随者",
    ["Cult Fanatic"] = "教派狂热者",
    ["Rampart of Skulls"] = "颅骨之墙",
    ["Muradin Bronzebeard"] = "穆拉丁·铜须",
    ["High Overlord Saurfang"] = "萨鲁法尔大王",
    ["Deathbringer's Rise"] = "死亡使者的高台",
    ["Deathbringer Saurfang"] = "死亡使者萨鲁法尔",
    ["Blood Beast"] = "血兽",
    ["The Plagueworks"] = "天灾工厂",
    ["Festergut"] = "烂肠",
    ["Rotface"] = "腐面",
    ["Little Ooze"] = "小软泥怪",
    ["Putricide's Laboratory of Alchemical Horrors and Fun"] = "普崔赛德的恐怖和娱乐化学实验室",
    ["Volatile Ooze"] = "不稳定的软泥怪",
    ["Professor Putricide"] = "普崔塞德教授",
    ["Gas Cloud"] = "毒气之云",
    ["The Crimson Hall"] = "血色厅堂",
    ["Prince Taldaram"] = "塔达拉姆王子",
    ["Prince Keleseth"] = "凯雷塞斯王子",
    ["Prince Valanar"] = "瓦拉纳王子",
    ["The Sanctum of Blood"] = "鲜血秘室",
    ["Blood-Queen Lana'thel"] = "鲜血女王兰娜瑟尔",
    ["The Frostwing Halls"] = "霜翼大厅",
    ["Valithria Dreamwalker"] = "踏梦者瓦莉瑟瑞娅",
    ["Gluttonous Abomination"] = "贪食的憎恶",
    ["Risen Archmage"] = "复生的大法师",
    ["Suppresser"] = "镇压者",
    ["The Frost Queen's Lair"] = "冰霜女王的巢穴",
    ["Sindragosa"] = "辛达苟萨",
    ["The Frozen Throne"] = "冰封王座",
    ["The Lich King"] = "巫妖王",
    ["Shambling Horror"] = "蹒跚的血僵尸",
    ["Ice Sphere"] = "寒冰之球",
    ["Raging Spirit"] = "暴怒的灵魂",
    ["Val'kyr Shadowguard"] = "瓦格里暗影戒卫者",
    ["The Ruby Sanctum"] = "红玉圣殿",
    ["Halion"] = "海里昂",
    ["Living Inferno"] = "活体地狱火",
    ["Living Ember"] = "活体余烬",

    -- T3 (泰坦怀旧时光 P3): 纳克萨玛斯 / 黑曜石圣殿 / 永恒之眼
    -- Naxxramas wings (subzones)
    ["The Arachnid Quarter"]   = "蜘蛛区",
    ["The Plague Quarter"]     = "瘟疫区",
    ["The Military Quarter"]   = "军事区",
    ["The Construct Quarter"]  = "构造区",
    ["Sapphiron's Lair"]       = "萨菲隆之巢",
    ["Kel'Thuzad's Lair"]      = "克尔苏加德的大厅",
    ["Loatheb's Chamber"]      = "死灵穹顶",
    ["Thaddius's Chamber"]     = "复活大厅",
    -- Spider wing
    ["Anub'Rekhan"]            = "阿努布雷坎",
    ["Grand Widow Faerlina"]   = "黑女巫法琳娜",
    ["Maexxna"]                = "迈克斯纳",
    -- Plague wing
    ["Noth the Plaguebringer"] = "瘟疫使者诺斯",
    ["Heigan the Unclean"]     = "肮脏的希尔盖",
    ["Loatheb"]                = "洛欧塞布",
    -- Military wing
    ["Instructor Razuvious"]   = "教官拉苏维奥斯",
    ["Gothik the Harvester"]   = "收割者戈提克",
    ["The Horsemen's Assembly"] = "骑士议会厅",
    ["Baron Rivendare"]        = "瑞文戴尔男爵",
    ["Thane Korth'azz"]        = "塞恩·寇斯加",
    ["Lady Blaumeux"]          = "布劳缪克斯小姐",
    ["Sir Zeliek"]             = "泽利克爵士",
    -- Construct wing
    ["Patchwerk"]              = "帕奇维克",
    ["Grobbulus"]              = "格罗布鲁斯",
    ["Gluth"]                  = "格拉斯",
    ["Thaddius"]               = "塔迪乌斯",
    ["Feugen"]                 = "费尔根",
    ["Stalagg"]                = "斯塔拉格",
    -- Frostwyrm wing
    ["Sapphiron"]              = "萨菲隆",
    ["Kel'Thuzad"]             = "克尔苏加德",
    -- Obsidian Sanctum
    ["The Obsidian Sanctum"]   = "黑曜石圣殿",
    ["Sartharion"]             = "萨塔里奥",
    ["Tenebron"]               = "塔纳布隆",
    ["Shadron"]                = "沙德隆",
    ["Vesperon"]               = "瓦斯佩隆",
    -- Eye of Eternity
    ["The Eye of Eternity"]    = "永恒之眼",
    ["Malygos"]                = "玛里苟斯",

    -- T4 P4 新增：祖尔格拉布 (Vanilla 20 人团本) — 译名按国服怀旧服官方
    ["Zul'Gurub"] = "祖尔格拉布",
    ["High Priestess Jeklik"]    = "高阶祭司耶克里克",
    ["High Priest Venoxis"]      = "高阶祭司温诺希斯",
    ["High Priestess Mar'li"]    = "高阶祭司玛尔里",
    ["Bloodlord Mandokir"]       = "血领主曼多基尔",
    ["Ohgan"]                    = "奥根",
    ["Gri'lek"]                  = "格里雷克",
    ["Hazza'rah"]                = "哈扎拉尔",
    ["Renataki"]                 = "雷纳塔基",
    ["Wushoolay"]                = "乌苏雷",
    ["High Priest Thekal"]       = "高阶祭司塞卡尔",
    ["Zealot Lor'Khan"]          = "狂热者洛卡恩",
    ["Zealot Zath"]              = "狂热者扎斯",
    ["Gahz'ranka"]               = "加兹兰卡",
    ["High Priestess Arlokk"]    = "高阶祭司娅尔罗",
    ["Jin'do the Hexxer"]        = "妖术师金度",
    ["Shade of Jin'do"]          = "金度的影像",
    ["Hakkar"]                   = "哈卡",
    ["Son of Hakkar"]            = "哈卡之子",

    -- T2 (Tempest Keep + Serpentshrine Cavern)
    ["Serpentshrine Cavern"] = "毒蛇神殿",
    ["The Lurker Below"] = "深水领主卡拉瑟雷斯",
    ["Hydross the Unstable"] = "不稳定的海度斯",
    ["Leotheras the Blind"] = "盲眼者莱欧瑟拉斯",
    ["Fathom-Lord Karathress"] = "鱼斯拉",
    ["Morogrim Tidewalker"] = "莫洛格里·踏潮者",
    ["Lady Vashj"] = "瓦丝琪",
    ["The Eye"] = "风暴要塞",
    ["Tempest Keep"] = "风暴要塞",
    ["Al'ar"] = "奥",
    ["Void Reaver"] = "空灵机甲",
    ["High Astromancer Solarian"] = "大星术师索兰莉安",
    ["Kael'thas Sunstrider"] = "凯尔萨斯·逐日者",

    -- T5 (泰坦怀旧时光 P5): 祖阿曼 / 太阳之井高地 — 区域名/boss 名取自客户端数据库
    -- (build 3.80.1.68768 DungeonEncounter/AreaTable)，小怪按 TBC 原版官方译名
    ["Zul'Aman"] = "祖阿曼",
    ["Akil'zon"]                  = "埃基尔松",
    ["Soaring Eagle"]             = "翱翔的雄鹰",
    ["Nalorakk"]                  = "纳洛拉克",
    ["Jan'alai"]                  = "加亚莱",
    ["Amani'shi Hatcher"]         = "阿曼尼孵化者",
    ["Amani Dragonhawk Hatchling"] = "阿曼尼龙鹰幼崽",
    ["Halazzi"]                   = "哈尔拉兹",
    ["Spirit of the Lynx"]        = "山猫之灵",
    ["Corrupted Lightning Totem"] = "腐化闪电图腾",
    ["Hex Lord Malacrass"]        = "妖术领主玛拉卡斯",
    ["Alyson Antille"]            = "阿莱松·安提雷",
    ["Thurg"]                     = "索尔格",
    ["Slither"]                   = "滑行者",
    ["Lord Raadan"]               = "兰尔丹",
    ["Gazakroth"]                 = "卡扎克洛斯",
    ["Fenstalker"]                = "沼泽猎手",
    ["Darkheart"]                 = "黑心",
    ["Koragg"]                    = "库拉格",
    ["Zul'jin"]                   = "祖尔金",
    ["Sunwell Plateau"] = "太阳之井高地",
    ["Kalecgos"]                  = "卡雷苟斯",
    ["Sathrovarr the Corruptor"]  = "腐蚀者萨索瓦尔",
    ["Brutallus"]                 = "布鲁塔卢斯",
    ["Felmyst"]                   = "菲米丝",
    ["Unyielding Dead"]           = "顽强的死尸",
    ["Lady Sacrolash"]            = "萨洛拉丝女王",
    ["Grand Warlock Alythess"]    = "高阶术士奥蕾塞丝",
    ["M'uru"]                     = "穆鲁",
    ["Entropius"]                 = "熵魔",
    ["Shadowsword Berserker"]     = "暗誓狂暴者",
    ["Shadowsword Fury Mage"]     = "暗誓怒火法师",
    ["Void Sentinel"]             = "虚空戒卫",
    ["Kil'jaeden"]                = "基尔加丹",
    ["Hand of the Deceiver"]      = "基尔加丹之手",
    ["Sinister Reflection"]       = "邪恶镜像",
    ["Shield Orb"]                = "护盾宝珠",
})
